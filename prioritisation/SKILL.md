...
echo '{"skill":"pm-prioritisation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pmstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
- After `/pm-office-hours` (Strategy mode) to score and sequence the initiative inventory
...
- If the top initiative has no brief: "Next: `/pm-office-hours` to start discovery on [initiative name]."
- If it has a brief but no CPO review: "Next: `/pm-cpo-review` for [initiative name]."
