...
echo '{"skill":"spec-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pmstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
- After `/plan-stakeholder-review` (the stakeholder objections tell you where the spec is thin)
- After `/prototype` — the test results define what the spec must deliver
