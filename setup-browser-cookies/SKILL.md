...
echo '{"skill":"setup-browser-cookies","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pmstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
B=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/pmstack/browse/dist/browse" ] && B="$_ROOT/.claude/skills/pmstack/browse/dist/browse"
[ -z "$B" ] && B=~/.claude/skills/pmstack/browse/dist/browse
if [ -x "$B" ]; then
  echo "READY: $B"
else
  echo "NEEDS_SETUP"
...
If `NEEDS_SETUP`:
1. Tell the user: "PMStack browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
