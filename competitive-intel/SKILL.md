...
echo '{"skill":"pm-competitive-intel","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.pmstack/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
- Before `/pm-office-hours` (Research mode) when the PM wants to understand the existing landscape before framing the problem
- After `/pm-office-hours` when the Product Brief needs competitive context to validate or challenge the opportunity
...
Report completion status. Then suggest: "Next: if this changes the problem framing, return to `/pm-office-hours` to update the Brief. If the framing holds, proceed to `/pm-assumption-audit` — several competitive observations likely become explicit assumptions that need testing."
